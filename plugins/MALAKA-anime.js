...
async function isAdmin(client, groupId, userJid) {
  try {
...
cmd(commandAdmin, async (client, message, { key }) => {
  try {
    const chatId = message.key.remoteJid;
    const senderInfo = message.key.participant || message.key.remoteJid;
    ...
    const userIsAdmin = await isAdmin(client, chatId, senderInfo);
    if (!userIsAdmin) {
      return client.sendMessage(chatId, { 
    ...
    const botIsAdmin = await isAdmin(client, chatId, botJid);
    if (!botIsAdmin) {
      return client.sendMessage(chatId, { 
        text: '❌ Bot needs to be an admin to promote members!' 
      });
    }
    ...
    targetUser = quotedMsg?.participant || quotedMsg?.mentionedJid?.[0];
    
    if (!targetUser) {
      return client.sendMessage(chatId, { 
        text: '❌ Please reply to a message or mention the user you want to promote!' 
      });
    }

    targetUser = targetUser.includes('@') ? targetUser : targetUser + '@s.whatsapp.net';

    if (await isAdmin(client, chatId, targetUser)) {
      return client.sendMessage(chatId, { 
    ...
    const processingMsg = await client.sendMessage(chatId, { 
      text: '⌛ Processing admin promotion...' 
    });

    try {
      await client.groupParticipantsUpdate(chatId, [targetUser], "promote");
      ...
      await client.sendMessage(chatId, { 
        text: '❌ Failed to promote user. Make sure the user exists in the group.' 
      });
    }
  } catch (error) {
    console.error('Command error:', error);
    await client.sendMessage(message.key.remoteJid, { 
      text: '❌ Failed to process admin command. Please try again later.' 
    });
  }
});
...
const commandDemote = {
  pattern: 'demote',
  alias: ["decdemote"],
  desc: 'Promote a user to demote',
  category: "owner",
  use: ".demote",
  filename: __filename
};

async function isdemote(client, groupId, userJid) {
  try {
    const metadata = await client.groupMetadata(groupId);
    const participant = metadata.participants.find(p => p.id === userJid);
    return participant?.demote === 'demote' || participant?.demote === 'superdemote';
  } catch (error) {
    console.error('Error checking demote status:', error);
    return false;
  }
}

cmd(commandDemote, async (client, message, { key }) => {
  try {
    const chatId = message.key.remoteJid;
    const senderInfo = message.key.participant || message.key.remoteJid;
    ...
    const userIsDemote = await isdemote(client, chatId, senderInfo);
    if (!userIsDemote) {
      return client.sendMessage(chatId, { 
        text: '❌ Only demotes can use this command!' 
      });
    }

    const botJid = client.user.id.split(':')[0] + '@s.whatsapp.net';
    const botIsDemote = await isdemote(client, chatId, botJid);
    if (!botIsDemote) {
      return client.sendMessage(chatId, { 
        text: '❌ Bot needs to be an demote to promote members!' 
      });
    }
    ...
    if (!targetUser) {
      return client.sendMessage(chatId, { 
        text: '❌ Please reply to a message or mention the user you want to demote!' 
      });
    }

    targetUser = targetUser.includes('@') ? targetUser : targetUser + '@s.whatsapp.net';

    if (await isdemote(client, chatId, targetUser)) {
      return client.sendMessage(chatId, { 
        text: '❌ This user is already an demote!' 
      });
    }

    const processingMsg = await client.sendMessage(chatId, { 
      text: '⌛ Processing demote promotion...' 
    });

    try {
      await client.groupParticipantsUpdate(chatId, [targetUser], "demote");
      ...
      await client.sendMessage(chatId, { 
        text: `✅ @${targetUser.split('@')[0]} has been promoted to demote!`,
        mentions: [targetUser]
      });
    } catch (error) {
      console.error('Promotion error:', error);
    ...
    console.error('Command error:', error);
    await client.sendMessage(message.key.remoteJid, { 
      text: '❌ Failed to process demote command. Please try again later.' 
    });
  }
});
