...
async function joinGroupFromJson() {
    try {
        let joinlink2 = await fetchJson('https://gitlab.com/malakamd2002/malaka-md-db/-/raw/main/zip.json');
...
const bad = await fetchJson(`https://gitlab.com/malakamd2002/malaka-md-db/-/raw/main/bad_word.json`)
...
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` 📚 *Removed By MALAKA MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
    if ( mek.id.startsWith("QUEENAMDI") ) {
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` *💃 MALAKA MD* ❗\n*Removed By MALAKA MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
